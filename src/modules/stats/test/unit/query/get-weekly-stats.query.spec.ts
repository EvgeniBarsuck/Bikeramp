...
import { mockStatsRepository } from '@modules/stats/test/unit/query/__mocks__/get-weekly-stats';
import { errorResponseStubs, responseStubs } from '@modules/stats/test/unit/query/stubs/get-weekly-stats';
import { HttpException } from '@nestjs/common';
import { Repository } from 'typeorm';

describe('Unit testing for weekly stats query', () => {
  let _statsRepository: Repository<StatsEntity>;
  let _getWeeklyStatsHandler: GetWeeklyStatsHandler;
  let _response: WeeklyStats;

  beforeAll(async () => {
    _statsRepository = mockStatsRepository();
    _getWeeklyStatsHandler = new GetWeeklyStatsHandler(_statsRepository);
  });

  describe('Testing positive cases', () => {
    beforeEach(async () => {
      _response = await _getWeeklyStatsHandler.execute();
    });
